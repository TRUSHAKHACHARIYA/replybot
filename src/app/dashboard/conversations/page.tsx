...
export default function ConversationsPage() {
  ...
  const handleSelect = async (conv: Conversation) => {
    ...
    if (conv.messages && conv.messages.length > 0) {
      ...
      try {
        const res = await fetch(`/api/conversations?conversation_id=${conv.id}`);
        ...
        {["All", "Active", "Escalated", "Resolved"].map((filter) => (
          <button key={filter}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
              filter === "All" ? "bg-primary-500 text-white" : "bg-surface-card border border-border text-text-secondary hover:bg-surface"
            }`}>
            {filter}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" style={{ minHeight: "calc(100vh - 220px)" }}>
        <div className="bg-surface-card rounded-xl border border-border overflow-hidden">
          ...
          <div className="divide-y divide-border max-h-[calc(100vh-320px)] overflow-y-auto">
            {loading ? (
              ...
              <p className="p-8 text-center text-sm text-text-muted">No conversations yet</p>
            ) : (
        ...
        <div className="lg:col-span-2 bg-surface-card rounded-xl border border-border flex flex-col">
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            ...
            {selectedConv && (
              <div className="flex items-center gap-2">
                <button className="px-3 py-1.5 rounded-lg text-xs font-medium bg-accent-50 text-accent-600 hover:bg-accent-100 transition-colors">Mark Resolved</button>
                <button className="px-3 py-1.5 rounded-lg text-xs font-medium bg-amber-50 text-amber-600 hover:bg-amber-100 transition-colors">Escalate</button>
              </div>
            )}
          </div>
              ...
              chatMessages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={`flex gap-3 ${isBot ? "" : "flex-row-reverse"}`}>
                    ...
                    }`}>{isBot ? "AI" : (selectedConv?.customer_name?.charAt(0) || "C")}</div>
          ...
          <div className="px-6 py-4 border-t border-border">
            <div className="flex gap-3">
              ...
              <button className="rounded-lg bg-primary-500 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-600 transition-colors">Send</button>
            </div>
          </div>
        </div>
      </div>
